...
FMOD_RESULT fetchDriver(FMOD::System *system, int *driver)
{
    ...
    do
    {
        ...
        for (int i = 0; i < numdrivers; i++)
        {
            ...
            result = system->getDriverInfo(i, name, 0, sizeof(name), 0, 0, 0, 0);
            ERRCHECK(result);
        ...
        Common_Sleep(50);
    } while (!Common_BtnPress(BTN_ACTION1));
...
int FMOD_Main()
{
    ...
    */
    result = systemA->createSound(Common_MediaPath("drumloop.wav"), FMOD_HARDWARE | FMOD_LOOP_OFF, 0, &soundA);
    ERRCHECK(result);

    result = systemB->createSound(Common_MediaPath("jaguar.wav"), FMOD_HARDWARE, 0, &soundB);
    ERRCHECK(result);
