/* ========================================================================================== */
/* FMOD Studio - DSP header file. Copyright (c), Firelight Technologies Pty, Ltd. 2004-2013.  */
/*                                                                                            */
...
typedef enum
{
    FMOD_DSP_PROCESS_PERFORM = 0,               /* Process the incoming audio in 'inbufferarray' and output to 'outbufferarray'. */
...
typedef FMOD_RESULT (F_CALLBACK *FMOD_DSP_PROCESS_CALLBACK)        (FMOD_DSP_STATE *dsp_state, unsigned int length, const FMOD_DSP_BUFFER_ARRAY *inbufferarray, FMOD_DSP_BUFFER_ARRAY *outbufferarray, int inputsidle, FMOD_DSP_PROCESS_OPERATION op);
...
typedef FMOD_RESULT (F_CALLBACK *FMOD_DSP_SETPARAM_BOOL_CALLBACK)  (FMOD_DSP_STATE *dsp_state, int index, int value);
...
typedef FMOD_RESULT (F_CALLBACK *FMOD_DSP_GETPARAM_BOOL_CALLBACK)  (FMOD_DSP_STATE *dsp_state, int index, int *value, char *valuestr);
...
typedef FMOD_RESULT (F_CALLBACK *FMOD_DSP_SHOULDIPROCESS)          (FMOD_DSP_STATE *dsp_state, int inputsidle, unsigned int length, FMOD_CHANNELMASK inmask, int inchannels, FMOD_SPEAKERMODE speakermode);

typedef FMOD_RESULT (F_CALLBACK *FMOD_DSP_SYSTEM_GETSAMPLERATECALLBACK) (FMOD_DSP_STATE* thisdsp, int *rate);
typedef FMOD_RESULT (F_CALLBACK *FMOD_DSP_SYSTEM_GETBLOCKSIZECALLBACK)  (FMOD_DSP_STATE* thisdsp, unsigned int *blocksize);
...
typedef enum
{
    FMOD_DSP_PARAMETER_FLOAT_MAPPING_TYPE_LINEAR = 0,		  /* Values mapped linearly across range. */
...
typedef struct FMOD_DSP_PARAMETER_DESC_INT
{
    ...
    int                      goestoinf;                /* [w] Whether the last value represents infiniy. */
...
{
    float linear_gain;                                  /* [r] The overall linear gain of the effect */
} FMOD_DSP_PARAMETER_OVERALLGAIN;
...
{
    int sidechainenable;                               /* [r/w] Whether sidechains are enabled. */
} FMOD_DSP_PARAMETER_SIDECHAIN;
...
#define FMOD_PLUGIN_SDK_VERSION 104
...
typedef struct FMOD_DSP_DESCRIPTION
{
    ...
    FMOD_DSP_SHOULDIPROCESS          shouldiprocess;     /* [w] This is called before processing.  You can detect if inputs are idle and return FMOD_OK to process, or any other error code to avoid processing the effect.  Use a count down timer to allow effect tails to process before idling! */
...
{
    FMOD_MEMORY_ALLOCCALLBACK               alloc;          /* [r] Memory allocation callback. Use this for all dynamic memory allocation within the plugin. */
    FMOD_MEMORY_REALLOCCALLBACK             realloc;        /* [r] Memory reallocation callback. */
    FMOD_MEMORY_FREECALLBACK                free;           /* [r] Memory free callback. */
    FMOD_DSP_SYSTEM_GETSAMPLERATECALLBACK   getsamplerate;  /* [r] Callback for getting the system samplerate. */
    FMOD_DSP_SYSTEM_GETBLOCKSIZECALLBACK    getblocksize;   /* [r] Callback for getting the system's block size.  DSPs will be requested to process blocks of varying length up to this size.*/
};
...
struct FMOD_DSP_STATE
{
    ...
    FMOD_CHANNELMASK                channelmask;         /* [r/w] Specifies which speakers the DSP effect is active on */
    FMOD_SPEAKERMODE                source_speakermode;  /* [r/w] Specifies which speaker mode the signal originated for information purposes, ie in case panning needs to be done differently. */
    float                          *sidechaindata;       /* [r/w] The mixed result of all incoming sidechains is stored at this pointer address. */
    int                             sidechainchannels;   /* [r] The number of channels of pcm data stored within the sidechain buffer. */
    struct FMOD_DSP_STATE_SYSTEMCALLBACKS *callbacks;           /* [r] Struct containing callbacks for system level functionality. */
};
...
typedef enum
{
    FMOD_DSP_ENVELOPEFOLLOWER_ATTACK,      /* (Type:float) - Attack time (milliseconds), in the range from 0.1 through 1000. The default value is 20. */
    FMOD_DSP_ENVELOPEFOLLOWER_RELEASE,     /* (Type:float) - Release time (milliseconds), in the range from 10 through 5000. The default value is 100 */
    FMOD_DSP_ENVELOPEFOLLOWER_ENVELOPE,    /* (Type:float) - Current value of the envelope, in the range 0 to 1. Read-only. */
    FMOD_DSP_ENVELOPEFOLLOWER_USESIDECHAIN /* (Type:bool)  - Whether to analyse the sidechain signal instead of the input signal. The default value is false */
