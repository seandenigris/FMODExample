...
/* ================================================================================================== */
/* FMOD Studio - Common C/C++ header file. Copyright (c), Firelight Technologies Pty, Ltd. 2004-2013. */
/*                                                                                                    */
...
#define FMOD_VERSION    0x00010208
...
typedef enum
{
    ...
    FMOD_ERR_SUBSOUNDS,             /* The error occured because the sound referenced contains subsounds when it shouldn't have, or it doesn't contain subsounds when it should have.  The operation may also not be able to be performed on a parent sound. */
    ...
    FMOD_ERR_EVENT_WONT_STOP,       /* The event cannot be released because it will not terminate, call stop to allow releasing of this event */
...
typedef enum
{
    ...
    FMOD_OUTPUTTYPE_AUDIOOUT,        /*                 - Audio Out output. */
...
#define FMOD_DEBUG_LEVEL_LOG            0x00000001      /* Enable informational messages. */
#define FMOD_DEBUG_LEVEL_WARNING        0x00000002      /* Enable warning messages. */
#define FMOD_DEBUG_LEVEL_ERROR          0x00000004      /* Enable error messages. */
...
#define FMOD_DEBUG_DISPLAY_LINENUMBERS  0x02000000      /* Display the FMOD Studio source code line numbers, for debugging purposes. */
...
typedef enum
{
    ...
    FMOD_SPEAKERMODE_FORCEINT = 65536  /* Makes sure this enum is 32bit. */
} FMOD_SPEAKERMODE;
...
#define FMOD_INIT_CHANNEL_LOWPASS            0x00000200 /* All FMOD_SOFTWARE (and FMOD_HARDWARE on 3DS and NGP) with FMOD_3D based voices will add a software lowpass filter effect into the DSP chain which is automatically used when Channel::set3DOcclusion is used or the geometry API.   This also causes sounds to sound duller when the sound goes behind the listener, as a fake HRTF style effect.  Use System::setAdvancedSettings to disable or adjust cutoff frequency for this feature. */
#define FMOD_INIT_CHANNEL_DISTANCEFILTER     0x00000400 /* All FMOD_SOFTWARE with FMOD_3D based voices will add a software lowpass and highpass filter effect into the DSP chain which will act as a distance-automated bandpass filter. Use System::setAdvancedSettings to adjust the center frequency. */
...
/* 
    FMOD Callbacks
*/
typedef FMOD_RESULT (F_CALLBACK *FMOD_SYSTEM_CALLBACK)          (FMOD_SYSTEM *system, FMOD_SYSTEM_CALLBACK_TYPE type, void *commanddata1, void *commanddata2);
...
typedef struct FMOD_CREATESOUNDEXINFO
{
    ...
    void                          *userdata;           /* [w] Optional. Specify 0 to ignore. This is user data to be attached to the sound during creation.  Access via Sound::getUserData.  Note: This is not passed to FMOD_FILE_OPENCALLBACK - use fileuserdata for that. */
